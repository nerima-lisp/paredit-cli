...
use crate::nested_string_case::usecase::{
    build_nested_string_case_report, evaluate_fail_on_violation_policy,
};
...
pub fn nested_string_case_report(args: NestedStringCaseReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_nested_string_case_report(file, dialect, &tree)?)
    });
    ...
    print_nested_string_case_report(&reports, &policy, args.output, args.verbosity)?;

    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "nested-string-case-report policy failed: {message}"
        )));
    }
