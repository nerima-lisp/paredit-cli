...
impl LintRule for Rule {
    ...
    fn check(
        ...
        examine_negation(view, &mut negation_form_count, &mut items);
        for item in items {
            let span = item.span;
...
mod tests {
    ...
    fn still_fires_inside_a_quasiquote_template() {
        assert_eq!(
            fixed("(defmacro m (a b) `(not (= ,a ,b)))\n"),
            vec!["(defmacro m (a b) `(/= ,a ,b))\n".to_owned()]
        );
    }
    ...
    fn a_form_carrying_its_own_hard_quote_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(not (= a b)))\n"), 0);
    }
    ...
    fn a_form_inside_a_quoted_ancestor_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(f (not (= a b))))\n"), 0);
    }
}
