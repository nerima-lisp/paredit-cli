...
use crate::nested_cxr::usecase::{build_nested_cxr_report, evaluate_fail_on_violation_policy};
...
pub fn nested_cxr_report(args: NestedCxrReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_nested_cxr_report(file, dialect, &tree)?)
    });
    ...
    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "nested-cxr-report policy failed: {message}"
        )));
    }
