...
(defclass Account []
  (defn __init__ [self id [balance 0]]
    (setv self.id id)
    (setv self.balance balance))

  (defn transfer [self other amount]
...
(defn summarise [accounts]
  ...
  (setv labels (lfor account accounts
                     :if account.balance
                     (.format "{}:{}" account.id account.balance)))
  (.format "{} (total {})" (.join ", " labels) total))

(defn load-config [path]
  (with [handle (open (Path path) "r")]
    (json.load handle)))

(defn retry [callback attempts]
