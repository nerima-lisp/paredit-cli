...
/// The hard-quote guard on the **fixable** numeric rules, driven end to end
/// through the real engine and asserted on the *rewritten source*.
///
/// A `Fixable` rule that fires inside hard-quoted data does not merely report a
/// false positive: `--fix` rewrites a user's data literal as if it were code,
/// and every round-trip property still passes, because a consistently wrong
/// parse is a fixed point of its own round-trip. So each rule below is pinned
/// in three directions at once, and the two that must *not* move are the point:
///
/// 1. hard-quoted data is left alone,
/// 2. a quasiquoted template is still fixed — and still carries its backquote,
/// 3. ordinary code is still fixed.
///
/// Every expected string here was generated by running [`run_rule_fixed`] — the
/// real dispatch, the real fix splice — rather than written by hand. Two of
/// them would have been wrong if they had been: `` `(equal ,x nil) `` rewrote to
/// `(null ,x)` with the backquote deleted, which leaves a comma outside a
/// backquote and stops the file reading at all.
#[cfg(test)]
mod fixable_quote_guard_tests {
    use paredit_core_lint_engine::rule::RuleEntry;

    use crate::support::run_rule_fixed;

    /// The rewritten source each fix produces, or an empty vector when the rule
    /// declined to fire at all.
    macro_rules! fixed_by {
        ($module:ident, $source:expr) => {{
            static ENTRIES: [RuleEntry; 1] = [RuleEntry::new(
                &crate::$module::rule::META,
                &crate::$module::rule::RULE,
            )];
    ...
    /// Asserts nothing was reported — the rule saw the form and declined.
    macro_rules! assert_silent {
        ($module:ident, $source:expr) => {
            assert_eq!(
                fixed_by!($module, $source),
                Vec::<String>::new(),
    ...
    /// Asserts the rule fired once and produced exactly `$expected`.
    macro_rules! assert_fixes_to {
        ($module:ident, $source:expr, $expected:expr) => {
            assert_eq!(
                fixed_by!($module, $source),
    ...
    /// 69 of this rule's 37 126 fixable findings over 28 304 Common Lisp files
    /// landed inside `'(…)`: ACL2 `:cases` proof hints, and the *expected
    /// expansion* an `assert-expands` compares against.
    #[test]
    fn sign_comparison_leaves_hard_quoted_data_alone() {
        assert_silent!(sign_comparison, "'(:cases ((< x 0)))");
        assert_silent!(sign_comparison, "'(when (= 0 (random 6)) (error \"b\"))");
    }

    #[test]
    fn sign_comparison_still_fires_inside_a_quasiquote_template() {
        assert_fixes_to!(sign_comparison, "`(< ,n 0)", "`(minusp ,n)");
        assert_fixes_to!(
            sign_comparison,
            "`(when (< ,n 0) (abort))",
            "`(when (minusp ,n) (abort))"
    ...
    #[test]
    fn sign_comparison_still_fires_on_ordinary_code() {
        assert_fixes_to!(
            sign_comparison,
            "(defun f (x) (< x 0))",
            "(defun f (x) (minusp x))"
    ...
    /// 60 of this rule's 6 194 fixable findings over the same corpus were
    /// whole function definitions embedded as quoted data in ACL2's
    /// `milawa2.lsp`.
    #[test]
    fn nil_comparison_leaves_hard_quoted_data_alone() {
        assert_silent!(nil_comparison, "'(cond ((equal x nil) t))");
        assert_silent!(nil_comparison, "'(true-listp (x) (equal x nil))");
    }

    /// The rewrite that was wrong before this module existed: the fix region
    /// was the form's `span`, which starts at the backquote, so applying it
    /// produced `(null ,x)` — a comma outside any backquote.
    #[test]
    fn nil_comparison_still_fires_inside_a_quasiquote_template() {
        assert_fixes_to!(nil_comparison, "`(equal ,x nil)", "`(null ,x)");
        assert_fixes_to!(
            nil_comparison,
            "`(when (equal ,x nil) t)",
            "`(when (null ,x) t)"
    ...
    #[test]
    fn nil_comparison_still_fires_on_ordinary_code() {
        assert_fixes_to!(
            nil_comparison,
            "(defun f (x) (equal x nil))",
            "(defun f (x) (null x))"
        );
    }
    ...
    /// 4 of this rule's 34 fixable findings over the same corpus sat inside
    /// `(eval-when (load) (eval '(defun … (incf *infix-loc* 1) …)))`.
    #[test]
    fn explicit_step_delta_leaves_hard_quoted_data_alone() {
        assert_silent!(explicit_step_delta, "(eval '(incf *loc* 1))");
    }

    #[test]
    fn explicit_step_delta_still_fires_inside_a_quasiquote_template() {
        assert_fixes_to!(explicit_step_delta, "`(incf ,place 1)", "`(incf ,place)");
    }

    #[test]
    fn explicit_step_delta_still_fires_on_ordinary_code() {
        assert_fixes_to!(
            explicit_step_delta,
            "(defun f () (incf *loc* 1))",
            "(defun f () (incf *loc*))"
    ...
    /// Preventive: this rule emitted no fix at all over the corpus, so its
    /// measured misfire count is 0 of 0. It is guarded because it shares both
    /// heads and its `examine_step` shape with `explicit-step-delta`, which
    /// does misfire on quoted data.
    #[test]
    fn negated_step_delta_leaves_hard_quoted_data_alone() {
        assert_silent!(negated_step_delta, "'(incf x -1)");
    }

    #[test]
    fn negated_step_delta_still_fires_inside_a_quasiquote_template() {
        assert_fixes_to!(negated_step_delta, "`(incf ,place -1)", "`(decf ,place 1)");
    }

    #[test]
    fn negated_step_delta_still_fires_on_ordinary_code() {
        assert_fixes_to!(
            negated_step_delta,
            "(defun f () (incf x -1))",
            "(defun f () (decf x 1))"
    ...
    /// Preventive: 0 of this rule's 1 427 fixable findings over the corpus were
    /// hard-quoted.
    #[test]
    fn redundant_divisor_leaves_hard_quoted_data_alone() {
        assert_silent!(redundant_divisor, "'(floor x 1)");
    }

    #[test]
    fn redundant_divisor_still_fires_inside_a_quasiquote_template() {
        assert_fixes_to!(redundant_divisor, "`(floor ,x 1)", "`(floor ,x)");
    }

    #[test]
    fn redundant_divisor_still_fires_on_ordinary_code() {
        assert_fixes_to!(
            redundant_divisor,
            "(defun f (x) (floor x 1))",
            "(defun f (x) (floor x))"
    ...
    /// Preventive: 0 of this rule's 2 788 fixable findings over the corpus were
    /// hard-quoted. It shares its `*` head with `single-operand-arithmetic`,
    /// whose hard-quoted findings are alien type specifiers rather than
    /// products.
    #[test]
    fn verbose_negation_leaves_hard_quoted_data_alone() {
        assert_silent!(verbose_negation, "'(- 0 x)");
    }

    #[test]
    fn verbose_negation_still_fires_inside_a_quasiquote_template() {
        assert_fixes_to!(verbose_negation, "`(- 0 ,x)", "`(- ,x)");
    }

    #[test]
    fn verbose_negation_still_fires_on_ordinary_code() {
        assert_fixes_to!(
            verbose_negation,
            "(defun f (x) (- 0 x))",
            "(defun f (x) (- x))"
    ...
    // -- the divergence the guard depends on ----------------------------------

    /// **The mutant this kills.** Guarding on a combined "is this data"
    /// predicate — `hard || quasi > 0` — is the obvious wrong fix, and it is
    /// wrong because a quasiquoted template is real code. Every `still_fires`
    /// assertion above fails under that mutant; this one names it.
    #[test]
    fn a_quasiquote_is_not_treated_as_hard_quoted_data() {
        assert_fixes_to!(sign_comparison, "`(< ,n 0)", "`(minusp ,n)");
        assert_fixes_to!(verbose_negation, "`(- 0 ,x)", "`(- ,x)");
        assert_fixes_to!(nil_comparison, "`(equal ,x nil)", "`(null ,x)");
        assert_fixes_to!(redundant_divisor, "`(floor ,x 1)", "`(floor ,x)");
    ...
    /// A comma inside a **hard** quote is a literal comma in a literal list,
    /// not an escape back to code — the shape a single depth counter gets
    /// wrong. Still data, so still silent.
    #[test]
    fn a_comma_inside_a_hard_quote_stays_data() {
        assert_silent!(nil_comparison, "'(a ,(equal x nil))");
        assert_silent!(sign_comparison, "'(a ,(< x 0))");
    }

    /// The long-hand spelling the reader also produces for `'…`. A guard that
    /// only looks at `ReaderPrefix::Quote` misses it.
    #[test]
    fn the_long_hand_quote_form_is_data_too() {
        assert_silent!(nil_comparison, "(quote (equal x nil))");
        assert_silent!(sign_comparison, "(quote (< x 0))");
        assert_silent!(verbose_negation, "(quote (- 0 x))");
    }
}
