...
mod tests {
    ...
    fn result(
        ...
        RefactorCheckFileResult {
            ...
            output_hash: "out".to_owned(),
            expected_input_hash: expected_input.to_owned(),
            expected_output_hash: "out".to_owned(),
            output_parse_ok: true,
            expected_output_parse_ok: true,
        }
    }

    /// The point of deriving rather than storing: a caller could previously
    /// construct a result whose `input_hash_matches` said `true` beside two
    /// hashes that differ. There is now no way to write that down.
    #[test]
    fn hash_agreement_cannot_disagree_with_the_hashes() {
        assert!(result("h", "h", false, false).input_hash_matches());
        assert!(!result("h", "other", false, false).input_hash_matches());
        assert!(result("h", "other", false, false).stale());
    }

    /// Same for the manifest's recorded flags: the comparison is the fields,
    /// not a third copy of the answer.
    #[test]
    fn manifest_flags_match_is_the_comparison_not_a_stored_claim() {
        assert!(result("h", "h", true, true).manifest_flags_match());
        assert!(!result("h", "h", true, false).manifest_flags_match());
    }
}
