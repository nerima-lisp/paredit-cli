...
| J7 | `edit format` のカラム幅計算が全角文字（日本語コメント等）を正しく考慮しているかの監査 |
| J8 | 複数行文字列リテラル内のインデント保持ポリシーの明文化・設定化 |
| J9 | `#\|...\|#` ブロックコメントのインデント正規化オプション |

### 深掘り: J7 — カラム幅計算の全角文字監査

`--max-width`（既定80）が文字数ベースかバイト数ベースか、UTF-8 の全角文字（日本語
コメント等）を1文字=1カラムとして数えているか2カラムとして数えているかは、
このカタログの調査では実装まで確認できていない — まず監査すべきは「実装がどちらの
前提で書かれているか」であって、新機能というより既存動作の確認が先に立つ候補。
もし現状が「1文字=1カラム」（Rust の `str::chars().count()` 相当）だとすると、
全角コメントを含む行は視覚的な実測幅より詰まって見える誤カウントが起きる。修正の
選択肢は東アジア幅（East Asian Width, UAX #11）を考慮したカラム計算に切り替える
ことだが、既存の全ゴールデンテスト（`tests/fixtures/*`）の期待値が変わる可能性が
高く、`--max-width` を使うテストの再生成が伴う破壊的変更になりうる。
