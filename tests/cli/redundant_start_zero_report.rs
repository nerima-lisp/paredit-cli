...
fn cli_does_not_flag_nonzero() {
    ...
    cmd.arg("inspect")
        ...
        // The denominator is what separates "no redundant :start in one call"
...
fn cli_labels_a_dialect_the_rule_does_not_model() {
    let dir = fresh_temp_dir("redundant-start-zero-report-unmodelled");
    let file = dir.join("a.clj");
    fs::write(&file, "(find x seq :start 0)\n").expect("write a.clj");

    paredit()
        .args(["inspect", "redundant-start-zero", "--output", "json"])
        .arg(&file)
        .assert()
        .success()
...
#[test]
fn cli_redundant_start_zero_emits_sarif() {
    let dir = fresh_temp_dir("redundant-start-zero-report-sarif");
    let file = dir.join("a.lisp");
    fs::write(&file, "(find x seq :start 0)\n").expect("write a.lisp");

    paredit()
        .args(["inspect", "redundant-start-zero", "--output", "sarif"])
        .arg(&file)
        .assert()
        .success()
        .stdout(predicate::str::contains(
            "\"ruleId\": \"inspect/redundant-start-zero/redundant-start-zero\"",
        ))
        .stdout(predicate::str::contains(
            "find :start defaults to 0; drop the explicit :start 0",
        ));
}
