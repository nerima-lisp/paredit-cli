...
use crate::constant_when_test::usecase::{
    build_constant_when_test_report, evaluate_fail_on_violation_policy,
};
...
pub fn constant_when_test_report(args: ConstantWhenTestReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_constant_when_test_report(file, dialect, &tree)?)
    });
    ...
    print_constant_when_test_report(&reports, &policy, args.output, args.verbosity)?;

    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "constant-when-test-report policy failed: {message}"
        )));
    }
