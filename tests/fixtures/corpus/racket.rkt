...
(provide (struct-out account) classify summarise transfer)

(struct account (id [balance #:mutable]) #:transparent)

(define default-options (hash 'retries 3 'timeout 0.5))

(define (make-accounts ids) (for/list ([id (in-list ids)]) (account id 0)))
...
(define
 (transfer from to amount #:note [note ""] #:allow-overdraft? [overdraft? #f])
...
(define (summarise accounts)
        (define total (for/sum ([a (in-list accounts)]) (account-balance a)))
        (define labels
                (for/list
                 ([a (in-list accounts)] #:unless (zero? (account-balance a)))
...
(define (walk tree)
        (let loop
          ...
          (match nodes
                 ['() (reverse acc)]
                 [(cons (? list? head) tail)
                  (loop tail (append (walk head) acc))]
                 [(cons head tail)
