...
use crate::presentation::cli::args::{
    CopyArgs, CursorArgs, EditTargetArgs, FormatArgs, KillArgs, KillRingArgs, NavigateArgs,
    NewlineArgs, OutputFormat, RaiseArgs, ReindentArgs, RepairArgs, ReplaceArgs, TargetArgs,
    TransposeArgs, UnwrapPrefixArgs, WrapArgs, YankArgs, YankPlacement,
};
...
pub(in crate::presentation::cli) fn kill(args: KillArgs) -> Result<()> {
    ...
    if args.to_ring {
        // Exactly what `kill` removes, which is the form and not the comment
        // block above it. `copy --to-ring` is the one that takes the comments,
        // because that is what `copy` prints.
        push_to_ring(&args.kill_ring, &input, selection.text())?;
    }
...
pub(in crate::presentation::cli) fn unwrap_prefix(args: UnwrapPrefixArgs) -> Result<()> {
    let (input, dialect, tree) =
        read_input_dialect_and_tree(args.target.file, args.target.dialect)?;
    let selection = resolve_target(&tree, args.target.path.as_ref(), args.target.at)?;
    let rewritten = Edit::unwrap_prefix(&input.text, &tree, selection, args.all)?;
    let rewritten = Edit::normalize_changed_line_trivia(&input.text, rewritten, dialect)?;
    Ok(emit_document(
        &input, dialect, args.write, args.diff, rewritten,
    )?)
...
pub(in crate::presentation::cli) fn transpose(args: TransposeArgs) -> Result<()> {
    let (input, dialect, tree) =
        read_input_dialect_and_tree(args.target.file, args.target.dialect)?;
    let first = resolve_target(&tree, args.target.path.as_ref(), args.target.at)?;
    let second = match (args.with_path.as_ref(), args.with_at) {
        (Some(path), None) => tree.select_path(path)?,
        (None, Some(offset)) => tree.select_at(offset)?,
        (None, None) => return Err(ArgumentError::SecondTargetRequired.into()),
        (Some(_), Some(_)) => return Err(ArgumentError::SecondTargetAmbiguous.into()),
    };
...
pub(in crate::presentation::cli) fn navigate(args: NavigateArgs) -> Result<()> {
    ...
    let from = resolve_target(&tree, args.target.path.as_ref(), args.target.at)?;
...
pub(in crate::presentation::cli) fn copy(args: CopyArgs) -> Result<()> {
    let (input, _, tree) = read_input_dialect_and_tree(args.target.file, args.target.dialect)?;
    let selection = resolve_target(&tree, args.target.path.as_ref(), args.target.at)?;
    let copied = Edit::copy(&input.text, &tree, selection)?;
    if args.to_ring {
        push_to_ring(&args.kill_ring, &input, &copied)?;
    }
    print!("{copied}");
    Ok(())
}
...
fn push_to_ring(args: &KillRingArgs, input: &SourceInput, text: &str) -> Result<()> {
    let path = ring_path(args.ring.clone());
    let mut ring = read_ring(&path)?;
    ring.push(
        KillRingEntry {
            text: text.to_owned(),
            origin: input.file.as_ref().map(|file| file.display().to_string()),
        },
        args.ring_size,
    );
