...
mod tests {
    ...
    fn still_fires_on_ordinary_unquoted_code() {
        assert_eq!(
            fixed("(defun f (c) (cond (c (a) (b))))\n"),
            vec!["(defun f (c) (when c (a) (b)))\n".to_owned()]
        );
    }
    ...
    fn still_fires_inside_a_quasiquote_template() {
        assert_eq!(
            fixed("(defmacro m (c) `(cond (,c (a) (b))))\n"),
            vec!["(defmacro m (c) `(when ,c (a) (b)))\n".to_owned()]
        );
    }
    ...
    fn a_form_carrying_its_own_hard_quote_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(cond (c (a) (b))))\n"), 0);
    }
    ...
    fn a_form_inside_a_quoted_ancestor_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(f (cond (c (a) (b)))))\n"), 0);
    }
}
