...
use crate::constant_if_test::usecase::{
    build_constant_if_test_report, evaluate_fail_on_violation_policy,
};
...
pub fn constant_if_test_report(args: ConstantIfTestReportArgs) -> CommandResult {
    ...
    for file in &files {
        ...
        reports.push(build_constant_if_test_report(file, dialect, &tree)?);
    }
    ...
    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "constant-if-test-report policy failed: {message}"
        )));
    }
