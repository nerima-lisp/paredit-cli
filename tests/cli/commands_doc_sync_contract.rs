//! docs/src/project/feature-candidates.md section P4: `docs/src/reference/api.md` drifts from the
...
fn every_inspect_edit_refactor_config_leaf_has_a_commands_md_row() {
        ...
        .expect("capabilities commands")
    {
        ...
        let section = sections
            .get(&heading)
            .unwrap_or_else(|| panic!("docs/src/reference/api.md has no `## {heading}` section"));
    ...
    assert!(
        missing.is_empty(),
        "docs/src/reference/api.md is missing a row for: {missing:#?}\n\
         Add one to the matching table — see docs/src/project/development.md's \
