//! inspect symbol-index reporting across a set of files.

pub use crate::symbol_index_report::domain::{IndexEntry, build_symbol_index_report};
...
pub fn evaluate_fail_on_external_policy(
    ...
    let failing = reports
        .iter()
        .map(|report| report.retained(|entry| entry.category.is_none()))
        .collect::<Vec<_>>();
    ...
    let mut policy = ReportPolicy::fail_on_any(
        ...
        |report| {
            format!(
                "{} references {} symbol(s) nothing analyzed defines",
                report.path.display(),
                report.findings.len()
            )
        },
