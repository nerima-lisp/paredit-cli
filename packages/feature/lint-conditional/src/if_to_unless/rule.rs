...
impl LintRule for Rule {
    ...
    fn check(
        ...
        examine(view, &mut if_form_count, &mut items);
        for item in items {
            let span = item.span;
...
mod tests {
    ...
    fn still_fires_on_ordinary_unquoted_code() {
        assert_eq!(
            fixed("(defun f (c e) (if c nil e))\n"),
            vec!["(defun f (c e) (unless c e))\n".to_owned()]
        );
    }
    ...
    fn still_fires_inside_a_quasiquote_template() {
        assert_eq!(
            fixed("(defmacro m (c e) `(if ,c nil ,e))\n"),
            vec!["(defmacro m (c e) `(unless ,c ,e))\n".to_owned()]
        );
    }
    ...
    fn a_form_carrying_its_own_hard_quote_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(if c nil e))\n"), 0);
    }
    ...
    fn a_form_inside_a_quoted_ancestor_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(f (if c nil e)))\n"), 0);
    }
}
