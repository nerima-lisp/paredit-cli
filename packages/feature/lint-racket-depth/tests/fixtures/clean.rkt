...
(provide
 (contract-out [classify (-> any/c symbol?)]
               [tally (-> (listof exact-integer?) exact-integer?)])
 render-all)
...
(define describe
        (match-lambda [(list 'ok value) value]
...
(define tally
        (case-lambda [(xs) (tally xs 0)]
                     [(xs start)
                      (for/fold ([total start]) ([x (in-list xs)]) (+ total x))]))
