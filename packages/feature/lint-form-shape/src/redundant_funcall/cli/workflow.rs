...
use crate::redundant_funcall::usecase::{
    build_redundant_funcall_report, evaluate_fail_on_violation_policy,
};
...
pub fn redundant_funcall_report(args: RedundantFuncallReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_redundant_funcall_report(file, dialect, &tree)?)
    });
    ...
    print_redundant_funcall_report(&reports, &policy, args.output, args.verbosity)?;

    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "redundant-funcall-report policy failed: {message}"
        )));
    }
