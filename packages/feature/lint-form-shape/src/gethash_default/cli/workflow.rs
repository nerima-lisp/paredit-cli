...
use crate::gethash_default::usecase::{
    build_gethash_default_report, evaluate_fail_on_violation_policy,
};
...
pub fn gethash_default_report(args: GethashDefaultReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_gethash_default_report(file, dialect, &tree)?)
    });
    ...
    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "gethash-default-report policy failed: {message}"
        )));
    }
