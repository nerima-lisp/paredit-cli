...
(define (make-queue) (make-raw-queue '() '()))

(define (queue-empty? q) (and (null? (queue-front q)) (null? (queue-back q))))

;; `begin` sequencing two effects: doing its job.
(define
 (queue-push! q value)
 (begin (set-queue-back! q (cons value (queue-back q))) q))

;; A named `let` that actually recurs, in tail position.
(define
 (queue-length q)
 (let loop
   ((items (queue-front q)) (total 0))
   (if (null? items) total
...
(define
 ...
 (let* ((front (queue-front q))
        (head
         (if (pair? front) (car front)
           #f))
        (rest
         (if (pair? front) (cdr front)
           '())))
...
(define (symbolic? value) (or (eq? value 'none) (eq? value '()) (eq? value #f)))
...
(define (code-known? code codes) (memv code codes))

(define (entry-for n table) (assv n table))
