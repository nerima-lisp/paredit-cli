...
use crate::redundant_start_zero::usecase::{
    build_redundant_start_zero_report, evaluate_fail_on_violation_policy,
};
...
pub fn redundant_start_zero_report(args: RedundantStartZeroReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_redundant_start_zero_report(file, dialect, &tree)?)
    });
    ...
    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "redundant-start-zero-report policy failed: {message}"
        )));
    }
