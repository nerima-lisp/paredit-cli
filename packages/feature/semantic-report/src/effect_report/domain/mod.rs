...
fn walk(
        ...
        if dialect == Dialect::CommonLisp && head.eq_ignore_ascii_case("if") {
            if let Some(live) = resolve_if_branch(view, bindings, values) {
                ...
                if let Some(live) = live {
                    walk(
                        live, defined, bindings, values, dialect, purity, cause, calls,
                    );
                }
                return;
            }
    ...
    for child in &view.children {
        walk(
            child, defined, bindings, values, dialect, purity, cause, calls,
        );
    }
}
...
fn resolve_constant<'a>(
    ...
    constant_key(name).and_then(|key| values.constant_value(&key))
}
