...
//!
//! The analysis lives in [`crate::domain::cons_to_list_report`], which also backs the
//! standalone `inspect cons-to-list` command; this module only registers it with
//! the lint suite and phrases its findings.

use anyhow::Result;

use crate::domain::cons_to_list_report::examine_cons;
...
impl LintRule for Rule {
    ...
    fn check(
        ...
        examine_cons(view, context.path(), &mut cons_form_count, &mut items);
        for item in items {
            let span = item.span;
            let fix = {
                let item = item.clone();
