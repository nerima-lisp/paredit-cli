...
(defn load-config
      [path]
      (try (json.loads (.read-text (Path path)))
           (except [e FileNotFoundError] (print "no config at" path) {})
           (except [e json.JSONDecodeError] (print "bad json" e) {})
           (except [e OSError] (print "io error" e) {})))

;; A tuple of types, then a broader clause. Neither type is covered above.
(defn fetch
      [key store]
      (try (get store key)
           (except [e [KeyError IndexError]] None)
           (except [e Exception] (print "unexpected" e) None)))

;; Sibling types under one parent: neither covers the other.
(defn convert
      [text]
      (try (int text) (except [e ValueError] 0) (except [e TypeError] 0)))
...
(defn strict-load
      ...
      (try (load-config path)
           (except [e ConfigError] (sys.exit 2))
           (except [e OSError] (sys.exit 1))))
...
(defn best-effort
      [thunk]
      (try (thunk) (except [e ValueError] None) (except [] None)))
...
(defmacro with-logging [#* body]
  `(try ~@body
        (except [e Exception] (print "failed"))
        (except [e ValueError] (print "value"))))
