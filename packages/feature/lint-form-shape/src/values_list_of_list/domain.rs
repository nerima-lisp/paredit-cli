...
#[derive(Debug)]
pub struct ValuesListOfListSummary {
    pub values_list_form_count: usize,
    pub violations: Vec<ValuesListOfListItem>,
}

#[derive(Debug, Clone, Copy)]
pub struct ValuesListOfListPolicyOptions {
...
#[derive(Debug)]
pub struct ValuesListOfListPolicy {
...
/// Collects every `(values-list (list …))` across a whole file, along with the
/// total number of `values-list` forms scanned.
pub fn collect_values_list_of_lists(
    path: &Path,
    dialect: Dialect,
    tree: &SyntaxTree,
) -> LintResult<(usize, Vec<ValuesListOfListItem>)> {
...
#[must_use]
pub const fn summarize_values_list_of_lists(
    values_list_form_count: usize,
    violations: Vec<ValuesListOfListItem>,
) -> ValuesListOfListSummary {
    ValuesListOfListSummary {
        values_list_form_count,
        violations,
    }
}
...
mod tests {
    ...
    fn calls(input: &str) -> (usize, Vec<ValuesListOfListItem>) {
        let tree = SyntaxTree::parse_with_dialect(input, Dialect::CommonLisp).expect("parse input");
        collect_values_list_of_lists(&PathBuf::from("test.lisp"), Dialect::CommonLisp, &tree)
            .expect("collect values-list of lists")
    }
    ...
    fn ignores_non_common_lisp_dialects() {
        ...
        let (count, violations) =
            collect_values_list_of_lists(&PathBuf::from("app.clj"), Dialect::Clojure, &tree)
                .expect("collect values-list of lists");
        assert_eq!(count, 0);
    ...
    fn policy_fails_only_when_flag_set() {
        let (count, items) = calls("(values-list (list a b))");
        let summary = summarize_values_list_of_lists(count, items);

        let quiet = evaluate_values_list_of_list_policy(
            ValuesListOfListPolicyOptions::new(false),
            &summary,
        );
        assert!(quiet.passed);
        ...
        let strict =
            evaluate_values_list_of_list_policy(ValuesListOfListPolicyOptions::new(true), &summary);
