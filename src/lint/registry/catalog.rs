...
// Pin the registry-derived counts so metadata changes cannot silently alter
// command behavior.
const _: () = assert!(RULE_COUNT == 358);
// Fixability is policy metadata; pin its aggregate independently.
const _: () = assert!(fixable_count() == 100);
// Severity is independent of fixability and tags.
const _: () = assert!(warning_count() == 256);
const _: () = assert!(EXPERIMENTAL_RULES.is_empty());
// Pedantic membership is user-visible preset policy.
