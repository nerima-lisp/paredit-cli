...
mod tests {
    ...
    fn still_fires_inside_a_quasiquote_template() {
        assert_eq!(
            fixed("(defmacro m (c) `(if ,c nil t))\n"),
            vec!["(defmacro m (c) `(not ,c))\n".to_owned()]
        );
    }
    ...
    fn a_form_carrying_its_own_hard_quote_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(if c nil t))\n"), 0);
    }
    ...
    fn a_form_inside_a_quoted_ancestor_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(f (if c nil t)))\n"), 0);
    }
}
