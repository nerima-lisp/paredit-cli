...
mod tests {
    ...
    fn still_fires_on_ordinary_unquoted_code() {
        assert_eq!(
            fixed("(defun f (a b c) (when a (when b c)))\n"),
            vec!["(defun f (a b c) (when (and a b) c))\n".to_owned()]
        );
    }
    ...
    fn still_fires_inside_a_quasiquote_template() {
        assert_eq!(
            fixed("(defmacro m (a) `(when ,a (when b c)))\n"),
            vec!["(defmacro m (a) `(when (and ,a b) c))\n".to_owned()]
        );
    }
    ...
    fn a_form_carrying_its_own_hard_quote_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(when a (when b c)))\n"), 0);
    }
    ...
    fn a_form_inside_a_quoted_ancestor_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(f (when a (when b c))))\n"), 0);
    }
}
