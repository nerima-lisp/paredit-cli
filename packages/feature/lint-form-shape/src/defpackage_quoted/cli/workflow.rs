...
use crate::defpackage_quoted::usecase::{
    build_defpackage_quoted_report, evaluate_fail_on_violation_policy,
};
...
pub fn defpackage_quoted_report(args: DefpackageQuotedReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_defpackage_quoted_report(file, dialect, &tree)?)
    });
    ...
    print_defpackage_quoted_report(&reports, &policy, args.output, args.verbosity)?;

    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "defpackage-quoted-report policy failed: {message}"
        )));
    }
