...
#[derive(Debug)]
pub struct ButlastDefaultCountSummary {
    pub call_form_count: usize,
    pub violations: Vec<ButlastDefaultCountItem>,
}

#[derive(Debug, Clone, Copy)]
pub struct ButlastDefaultCountPolicyOptions {
    fail_on_violation: bool,
}

impl ButlastDefaultCountPolicyOptions {
    #[must_use]
...
#[derive(Debug)]
pub struct ButlastDefaultCountPolicy {
...
/// Collects every `(butlast list 1)` / `(nbutlast list 1)` across a whole file,
/// along with the total number of such calls scanned.
pub fn collect_butlast_default_counts(
    path: &Path,
    dialect: Dialect,
    tree: &SyntaxTree,
) -> LintResult<(usize, Vec<ButlastDefaultCountItem>)> {
...
#[must_use]
pub const fn summarize_butlast_default_counts(
    call_form_count: usize,
    violations: Vec<ButlastDefaultCountItem>,
) -> ButlastDefaultCountSummary {
    ButlastDefaultCountSummary {
        call_form_count,
        violations,
    }
...
#[must_use]
pub fn evaluate_butlast_default_count_policy(
    options: ButlastDefaultCountPolicyOptions,
    summary: &ButlastDefaultCountSummary,
) -> ButlastDefaultCountPolicy {
...
mod tests {
    ...
    fn calls(input: &str) -> (usize, Vec<ButlastDefaultCountItem>) {
        let tree = SyntaxTree::parse_with_dialect(input, Dialect::CommonLisp).expect("parse input");
        collect_butlast_default_counts(&PathBuf::from("test.lisp"), Dialect::CommonLisp, &tree)
            .expect("collect butlast default counts")
    }
    ...
    fn ignores_non_common_lisp_dialects() {
        ...
        let (count, violations) =
            collect_butlast_default_counts(&PathBuf::from("app.clj"), Dialect::Clojure, &tree)
                .expect("collect butlast default counts");
        assert_eq!(count, 0);
        assert!(violations.is_empty());
    }
    ...
    fn policy_fails_only_when_flag_set() {
        let (count, items) = calls("(butlast xs 1)");
        let summary = summarize_butlast_default_counts(count, items);

        let quiet = evaluate_butlast_default_count_policy(
            ButlastDefaultCountPolicyOptions::new(false),
            &summary,
        );
        assert!(quiet.passed);
        assert_eq!(quiet.violation_count, 1);

        let strict = evaluate_butlast_default_count_policy(
            ButlastDefaultCountPolicyOptions::new(true),
            &summary,
        );
