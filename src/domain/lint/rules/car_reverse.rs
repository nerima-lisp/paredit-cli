...
//!
//! The analysis lives in [`crate::domain::car_reverse_report`], which also backs the
//! standalone `inspect car-reverse` command; this module only registers it with
//! the lint suite and phrases its findings.

use anyhow::Result;

use crate::domain::car_reverse_report::examine;
...
impl LintRule for Rule {
    ...
    fn check(
        ...
        examine(view, context.path(), &mut accessor_form_count, &mut items);
        for item in items {
            let span = item.span;
            let fix = {
