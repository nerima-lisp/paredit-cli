...
use crate::one_step_arithmetic::usecase::{
    build_one_step_arithmetic_report, evaluate_fail_on_violation_policy,
};
...
pub fn one_step_arithmetic_report(args: OneStepArithmeticReportArgs) -> CommandResult {
    ...
    for file in &files {
        ...
        reports.push(build_one_step_arithmetic_report(file, dialect, &tree)?);
    }
    ...
    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "one-step-arithmetic-report policy failed: {message}"
        )));
    }
