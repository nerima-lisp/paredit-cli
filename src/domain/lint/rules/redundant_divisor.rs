...
//!
//! The analysis lives in [`crate::domain::redundant_divisor_report`], which also backs the
...
use crate::domain::redundant_divisor_report::examine;
...
impl LintRule for Rule {
    ...
    fn check(
        ...
        examine(view, context.path(), &mut quotient_form_count, &mut items);
        for item in items {
            let span = item.span;
            let fix = {
                let item = item.clone();
