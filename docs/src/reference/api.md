...
| `go-block-blocking-channel-op` | Report a core.async **blocking** op (`<!!`, `>!!`, `alts!!`, `alt!!`) reached from a `go` body. core.async's own docstring is the source: this "risks depleting the fixed pool of go block threads, causing all go block processing to stop" — a deadlock that takes the whole process's go machinery with it, not one call. Error severity, Clojure only, report-only (dropping one `!` is the repair most of the time and `(thread …)` the rest, which is a restructuring only the author can choose). |
| `parking-op-outside-go-machinery` | Report a core.async **parking** op (`<!`, `>!`, `alts!`, `alt!`) lexically inside a `go` body but behind a function or `thread` boundary the `go` transform does not rewrite. The op then expands to `(assert nil "<! used not in (go ...) block")` — a hard failure on first execution, or a silent `nil` when `*assert*` was false at compile time. Error severity, Clojure only, report-only (the repair is a restructuring: `doseq` for `for`, a nested `go`, an explicit loop). |
| `contains-on-non-associative` | Report a `contains?` whose collection is a sequence, or a literal vector asked about a non-index key — a membership test that can never answer `true`. One shape throws `IllegalArgumentException`; the other is silently and permanently `false`, which is the worse of the two. Error severity, Clojure only, report-only (`(some #{k} coll)`, a `set`, or restructuring the data are three repairs with three different costs). |
| `reference-type-operator-mismatch` | Report an atom/ref/volatile operator applied to a local bound by a *different* reference constructor — `swap!` on a `(ref …)`, `alter` on an `(atom …)` — which throws `ClassCastException` on the first call. Error severity, Clojure only, report-only (the sibling operator and the other constructor are both valid repairs, and which is right depends on what coordination the code needs). |
...
With 345 rules, `inspect lint` needs more than an on/off switch per rule. The
...
The 345 shipped rules are the ones everybody gets. A rule like "in *this*
