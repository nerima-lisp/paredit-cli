...
use crate::single_arg_comparison::usecase::{
    build_single_arg_comparison_report, evaluate_fail_on_violation_policy,
};
...
pub fn single_arg_comparison_report(args: SingleArgComparisonReportArgs) -> CommandResult {
    ...
    for file in &files {
        ...
        reports.push(build_single_arg_comparison_report(file, dialect, &tree)?);
    }
    ...
    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "single-arg-comparison-report policy failed: {message}"
        )));
    }
