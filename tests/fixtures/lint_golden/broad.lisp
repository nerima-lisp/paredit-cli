...
(progn
  ...
  (progn
    b
    c)) ; nested-progn (the inner progn)

(when q
  (progn
    s
    t)) ; redundant-body-progn
...
(let* ((a 1))
  a) ; redundant-let-star

(cond
  (ok (run))) ; single-clause-cond
...
(multiple-value-bind (mv) (vals)
  mv) ; single-value-bind
...
(defun scale (factor x factor)
  (* factor x)) ; duplicate-parameters

(defun dupkw (&optional x &optional y)
  x) ; duplicate-lambda-list-keyword

(defun kworder (&key a &optional b)
  a) ; lambda-list-keyword-order

(when (eq status status)
  1) ; self-comparison

(defun ready? (x)
  (eq (compute x) t)) ; t-comparison

(if test
    (foo x)
    (foo x)) ; identical-if-branches
...
(typecase x
  (nil 1)
  (t 2)) ; typecase-nil-key

(case x
  (1 :one)
  2
  :two) ; malformed-case-clause

(case x
  (1 :one)
  (t :def)
  (2 :two)) ; unreachable-case-clause

(ecase x
  (1 :one)
  (t :default)) ; exhaustive-case-otherwise

(cond
  ((foo) 1)
  ((bar) 2)
  ((foo) 3)) ; duplicate-cond-tests

(cond
  ((foo) 1)
  (t 2)
  ((bar) 3)) ; unreachable-cond-clause

(cond
  ((plusp x) :pos)
  ((minusp x) :neg)
  :zero) ; malformed-cond-clause

(let ((x 1)
      (y 2)
      (x 3))
  x) ; duplicate-let-bindings

(let ((x 1 y 2))
  (+ x y)) ; malformed-let-binding

(let ((nil 1)
      (:status :ok))
  :status) ; binds-constant

(dolist (x)
  (print x)) ; malformed-iteration-spec

(eval-when (:compile-toplevel :executee)
...
(when (eql name "root")
  1) ; eql-string-comparison

(when (eql p '(:a :b))
  1) ; eql-list-comparison

(defun eqlsearch (items)
  (member "x" items)) ; eql-search-literal

(defun singlearg (x)
  (when (< x)
    (go))) ; single-arg-comparison

(defun missingdest (x)
  (format "~a~%" x)) ; format-missing-destination

(defun litplace ()
  (incf 5)) ; literal-place

(defun destrlit ()
  (nreverse '(a b c))) ; destructive-literal

(defun charopstr (c)
  (when (char= c "a")
    :hit)) ; char-op-string

(defun emptybody (ready)
  (when ready)) ; empty-body

(defun identarith (x)
  (+ x 0)) ; identity-arithmetic
