...
use crate::make_array_default_keyword::usecase::{
    build_make_array_default_keyword_report, evaluate_fail_on_violation_policy,
};
...
pub fn make_array_default_keyword_report(args: MakeArrayDefaultKeywordReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_make_array_default_keyword_report(
            file, dialect, &tree,
        )?)
    });
    ...
    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "make-array-default-keyword-report policy failed: {message}"
        )));
    }
