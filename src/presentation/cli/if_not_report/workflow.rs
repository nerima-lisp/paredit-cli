...
pub(in crate::presentation::cli) fn if_not_report(args: IfNotReportArgs) -> Result<()> {
    ...
    for file in &files {
        ...
        let (file_form_count, file_violations) = collect_if_nots(file, dialect, &tree)?;
        if_form_count += file_form_count;
        violations.extend(file_violations);
    }

    let summary = summarize_if_nots(if_form_count, violations);
    let policy = evaluate_if_not_policy(IfNotPolicyOptions::new(args.fail_on_violation), &summary);
