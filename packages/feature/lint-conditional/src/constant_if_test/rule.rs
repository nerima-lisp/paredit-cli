...
impl LintRule for Rule {
    ...
    fn check(
        ...
        examine_if(view, &constant_test, &mut if_form_count, &mut items);
        for item in items {
            let span = item.span;
...
mod tests {
    ...
    fn a_form_carrying_its_own_hard_quote_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(if t a b))\n"), 0);
    }
    ...
    fn a_form_inside_a_quoted_ancestor_is_not_a_finding() {
        assert_eq!(count("(defparameter *p* '(f (if t a b)))\n"), 0);
    }
}
