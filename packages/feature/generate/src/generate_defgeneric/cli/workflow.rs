...
pub fn generate_defgeneric(args: GenerateDefgenericArgs) -> Result<()> {
    ...
    let candidates = match &args.name {
        Some(name) => find_by_name(&tree, name)
            ...
            .ok_or_else(|| anyhow::anyhow!("no defmethod named {name} without a defgeneric"))?,
    ...
    if rewritten != input.text {
        SyntaxTree::parse_with_dialect(&rewritten, dialect)
            .context("the generated defgeneric form(s) would leave the file unparseable")?;
    }
