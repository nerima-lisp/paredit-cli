...
//! pass that runs them.
//!
//! A rule declares which nodes it wants and what to report, [`registry`] is the
//! source list for the derived catalogue, and [`engine`] walks the document
//! once for all rules.
//!
...
// `engine`, `model`, `policy`, and `rule` live in `paredit-core-lint-engine`;
// `registry` and `rules` stay here because a
