...
use super::super::policy::{is_pure_standard_function, is_standard_control_form};

/// Whether `head` names a form whose semantics this layer knows.
///
/// Three sources count. [`CommonLispOperator`] registers the special and
/// definition forms — everything that binds. The standard-function table
/// covers ordinary calls: a function receives its arguments' values and has no
/// access to the caller's lexical environment, so it provably cannot reassign
/// a binding. The control-form table covers `if`, `progn`, `setq` and their
/// kin, which *can* reach a binding but evaluate their subforms where they are
/// written, leaving any assignment visible for the assignment collector.
///
