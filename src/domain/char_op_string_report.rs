...
//!
//! Only a *string literal* argument is flagged; a character literal, a symbol,
//! or any other form is left alone. This is the character-function sibling of
//! `eql-string-comparison`, which covers the same string/char confusion for
//! `eq`/`eql`.
//!
...
/// The text of the first string-literal argument (after the operator), or
/// `None` if no argument is a string literal.
fn first_string_argument(view: &ExpressionView) -> Option<&str> {
    view.children
        .iter()
        .skip(1)
        .find_map(|child| atom_text(child).filter(|text| text.starts_with('"')))
}
...
pub struct CharOpStringItem {
    ...
    pub operator: String,
    /// The offending string literal (its source text).
    pub literal: String,
}
...
fn examine_call(
    ...
    *char_call_count += 1;
    ...
    if let Some(literal) = first_string_argument(view) {
...
pub fn collect_char_op_strings(
    ...
    for index in 0..tree.root_children().len() {
        ...
        for_each_subview(&view, |subview| {
            examine_call(subview, path, &mut char_call_count, &mut violations)
        });
    }
