...
use crate::char_case_fold::usecase::{
    build_char_case_fold_report, evaluate_fail_on_violation_policy,
};
...
pub fn char_case_fold_report(args: CharCaseFoldReportArgs) -> CommandResult {
    ...
    for file in &files {
        ...
        reports.push(build_char_case_fold_report(file, dialect, &tree)?);
    }
    ...
    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "char-case-fold-report policy failed: {message}"
        )));
    }
