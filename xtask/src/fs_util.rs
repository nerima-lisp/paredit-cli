...
pub fn write_new_file(path: &Path, contents: &str) -> Result<()> {
    ...
    std::fs::write(path, contents).map_err(crate::error::XtaskError::io(format!("write {}", path.display())))?;
...
pub fn insert_sorted_mod_line(lib_rs: &Path, module_line: &str) -> Result<()> {
    ...
    let first = lines
        ...
        .map_err(crate::error::XtaskError::io(format!("{} has no `pub mod` line to anchor on", lib_rs.display())))?;
    let mut last = first;
    ...
    std::fs::write(lib_rs, new_text).map_err(crate::error::XtaskError::io(format!("write {}", lib_rs.display())))?;
