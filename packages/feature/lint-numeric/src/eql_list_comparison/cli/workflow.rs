...
use crate::eql_list_comparison::usecase::{
    build_eql_list_comparison_report, evaluate_fail_on_violation_policy,
};
...
pub fn eql_list_comparison_report(args: EqlListComparisonReportArgs) -> CommandResult {
    ...
    let analysis = analyze_files_raw(&files, |file| {
        ...
        CliResult::Ok(build_eql_list_comparison_report(file, dialect, &tree)?)
    });
    ...
    print_eql_list_comparison_report(&reports, &policy, args.output, args.verbosity)?;

    if !passed {
        return Err(paredit_core_cli::gate::gate_failure(format!(
            "eql-list-comparison-report policy failed: {message}"
        )));
    }
